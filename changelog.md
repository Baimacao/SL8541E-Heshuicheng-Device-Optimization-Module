...
- **动画两步法的间隔从 1 秒改成 6 秒**（用户实测：1 秒会失效，6 秒可行），
  并且把动画段挪到 `service.sh` 最后执行，避免长 sleep 挡住 WebUI 状态页生成
- **`module.prop` 增加 `updateJson` 声明** —— 这是根管理器「更新」按钮真正的入口。
  v1.4~v1.6 只放了 `update.json` 文件却没在 `module.prop` 里声明，管理器根本不知道去哪读，
  所以按钮一直不出现。地址用 `releases/latest/download/update.json`（固定不变、自动指向最新）
- **发布两种包**：
  - **HSC 特供**（`..._v1.7_HSC.zip`）：完整功能，含虚标 / 云控 / 指纹人脸等原厂定制处理
  - **通用**（`..._v1.7_Universal.zip`）：只做与机型无关的优化（dex2oat / TCP / 图形 / 日志 / 音频）
  - 差异通过 `lib/prop.list` 的 `variant` 列 + `lib/variant` 文件实现，**一份源码出两个包**
- 开机清理空文件夹（v1.6 引入）
## v1.6
